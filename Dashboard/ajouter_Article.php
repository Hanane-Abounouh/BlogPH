...
if (isset($_POST['add_article'])) {
    ...
    $id_utilisateur = 1; // ID de l'utilisateur admin
...
<section class="px-20 Ajouter py-5 flex flex-col justify-center items-center w-full">
    <div class="my-6">
        <div class="flex mt-5 gap-5 items-center text-gray-600 hover:text-gray-900">
            <img src="https://icons.veryicon.com/png/o/system/content-management-system-background/add-article.png" class="sFlh5c pT0Scc iPVvYb" style="max-width: 30px; height: 30px; width: 30px;" alt="icon-article Vector Icons free download in SVG, PNG Format">
        ...
        <div class="grid mt-2 sm:grid-cols-2 items-center gap-16 p-8 mx-auto max-w-4xl border border-gray-300 bg-white shadow-[0_2px_10px_-3px_rgba(6,81,237,0.3)] rounded-md text-[#333] font-[sans-serif]">
            <form method="post" enctype="multipart/form-data" class="ml-auo space-y-4">
                ...
                <div class="col-span-full">
                    <label for="sous_titre" class="block text-sm font-semibold leading-6 text-gray-900">Sous-titre</label>
                ...
                <div>
                    ...
                    <input type="file" name="image" id="image" class="rounded-md bg-gray px-2.5 py-1.5 text-sm font-semibold text-gray-900 shadow-sm ring-1 ring-inset ring-gray-600 hover:bg-gray-500">
                </div>
                <label for="contenu" class="block text-sm font-semibold leading-6 text-gray-900">Contenu</label>
                <textarea placeholder='Contenu' rows="6" id="contenu" name="contenu" class="w-full rounded-md px-4 border text-sm pt-2.5 outline-[#007bff]"></textarea>
                ...
                <button type="submit" name="add_article" class="text-white bg-[#E53902] hover:bg-orange-500 font-semibold rounded-md text-l px-4">Send</button>
            </form>
        </div>
    </div>
</section>
